# tried adding a walk component for a minute
# looks like it's going to take a lot of time to 
# adjust and get the walk right
# and we don't gain much (anything?) compared to just lumping the beginning and ending weeks
# so not pursuing further
...
modFile <- "PBT_models/model_pbt_strat_2_walk.txt"
cat("
    ...
    model{
   	...
   	for (w in 2:nWeeks){
   		for(g in 1:n_AD_groups){
   			log_AD_PBT[g,w] ~ dnorm(log_AD_PBT[g,w-1], AD_tau[g])
   		}
   		for(g in 1:n_AI_groups){
   			log_AI_PBT[g,w] ~ dnorm(log_AI_PBT[g,w-1], AI_tau[g])
   		}
   	}
   	...
   	for (w in 1:nWeeks){
   		...
   		p_clip[w] ~ dbeta(.01,.01)T(0.001,0.999)
   	}
		for (g in 1:n_AD_groups){
			log_AD_PBT[g,1] ~ dnorm(0,1.0E-3)
		}
		for (g in 1:n_AI_groups){
			log_AI_PBT[g,1] ~ dnorm(0,1.0E-3)
		}
...
cbind(apply(s1[[1]][,cols_same],2,median),
		apply(s2[[1]][,cols_same],2,median)
)
...
plot(s1[[1]][,"grand_tot_AI_groups[9]"])
